...
export const getFallbackFontName = (font) => {
    ...
    if (fallbackFontName === initial) {
        throw Error(`[common] fallback flag is not found in font. true fallback flag must be only one.`);
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onloadend = () => {
            if (reader.result.startsWith('data:application/pdf;')) {
                ...
                reject(Error('[common] template.basePdf must be pdf data.'));
            }
        };
...
export const checkFont = (arg) => {
    ...
    if (fallbackFontNum > 1) {
        throw Error(`[common] ${fallbackFontNum} fallback flags found in font. true fallback flag must be only one.`);
    }
    const fontNamesInSchemas = getFontNamesInSchemas(schemas);
    const fontNames = Object.keys(font);
    if (fontNamesInSchemas.some((f) => !fontNames.includes(f))) {
        throw Error(`[common] ${fontNamesInSchemas
...
export const checkPlugins = (arg) => {
    ...
    if (allSchemaTypes.some((s) => !pluginsSchemaTypes.includes(s))) {
        throw Error(`[common] ${allSchemaTypes
...
--------------------------`);
            ...
            throw Error(`[common] Invalid argument:
--------------------------
${message}`);
        }
    }
